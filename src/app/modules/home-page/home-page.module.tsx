...
export default function HomePage() {
  ...
  return (
    <main className='relative h-[calc(100vh-64px)] overflow-hidden px-5'>
        <Image src={bgImage} alt="Minimalistic Background" fill placeholder='blur' className='-z-10 object-cover' />
        <div className="font-sans flex flex-col items-center justify-center gap-10 h-full">
          ...
          <Link href='#' className={buttonVariants({ variant: 'default' })}>{translations('actions.viewProducts')}</Link>
        </div>
    </main>
  );
}
